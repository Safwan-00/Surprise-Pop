...
int pointCount = pointEasy = pointHard;
int pointShowX = 250;
int pointShowY = 410;
...
char balloon2[30] = "Pic\\ball\\balloon2.bmp";
char balloon5[30] = "Pic\\ball\\balloon5.bmp";
	...
	for (int i = 0; i < bulletCount ; i++)
		{
			for (int j = 0 ; j < 2 ; j++)
				{
					if(bulleteasy[i].hasActive && Ebomb[j].bombshow)
						{
							if ((bulleteasy[i].bull_x+40 > Ebomb[j].bx && bulleteasy[i].bull_x+40< Ebomb[j].bx+80)&&(bulleteasy[i].bull_y+20 >Ebomb[j].by && bulleteasy[i].bull_y+20 < Ebomb[j].by+80))
								{
									Ebomb[j].bombshow = false;
									bulleteasy[i].deactive=true;
									bullseyeEasy++;
									brustEasy++;
									bombEx = true;
									if(Ebomb[j].bombshow==false)
									{
										iShowBMP2(Ebomb[j].bx,Ebomb[j].by,BombExplosion[bombExIndex],0);
										pointEasy+=10;
									}
...
iText(pointShowX-10,pointShowY,"Score: ",GLUT_BITMAP_TIMES_ROMAN_24);
iSetColor(0,255,255);
iText(pointShowX+50,pointShowY,StrPoint,GLUT_BITMAP_TIMES_ROMAN_24);
}
//showing remaing bullet for easy
void bullnumbershow(){
	...
	for (int i = 0; i < bulletCountHard ; i++)
		{
			for (int j = 0 ; j < 2 ; j++)
				{
					if(bullethard[i].hasActive && Hbomb[j].bombshow)
						{
							if ((bullethard[i].hbull_x+40 > Hbomb[j].bx && bullethard[i].hbull_x+40< Hbomb[j].bx+80)&&(bullethard[i].hbull_y+20 >Hbomb[j].by && bullethard[i].hbull_y+20 < Hbomb[j].by+80))
								{
									bombEx = true;
									Hbomb[j].bombshow = false;
									bullethard[i].deactive=true;
									
									bullseyeHard++;
									brustHard++;
									
									if(Hbomb[j].bombshow==false)
									{
										iShowBMP2(Hbomb[j].bx,Hbomb[j].by,BombExplosion[bombExIndex],0);
										pointHard+=10;
									}
...
char khajana[25]="Pic\\Bomb\\k1.bmp";
char KhajanaOpen[2][25]={"Pic\\Bomb\\k2.bmp","Pic\\Bomb\\k3.bmp"};
int khajanaOpen=0;
int khajanaIndex=0;
bool kopen=true;
...
void KhajanaMove()
{
	for(int i=0;i<2;i++)
	{
		if(khaJana[i].khajanashow)
		{
			iShowBMP2(khaJana[i].kx,khaJana[i].ky,khajana,0);
...
void KhajanaChange()
{
	for(int i=0;i<2;i++)
	{
		khaJana[i].ky-=2;		//slow or fast
		if(khaJana[i].ky<=0)
		{
			khaJana[i].ky=screenHeight;
		}
...
void SetKhajana()
{
	for(int i=0;i<2;i++)
	{
		khaJana[i].kx=150+rand()%screenWidth;
		khaJana[i].ky=rand()%screenHeight;
		khaJana[i].khajanashow=true;
	}
}
...
{	
	...
	for(int i=0;i<2;i++)
	{
		if(pwactive==true && khaJana[i].khajanashow==true)
		{
		
			if ( (khaJana[i].ky > walkY && khaJana[i].ky+71 < walkY+160)   &&   (khaJana[i].kx >= walkX+15 && khaJana[i].kx+80 <= walkX+145) )
				{
					
					
							khaJana[i].khajanashow=false;
							 khajanaOpen++;
							 kopen=true;
							 kcollcount++;
							 if(khaJana[i].khajanashow==false)
							 {
									iShowBMP2(walkX+20,walkY+20,KhajanaOpen[khajanaIndex],0);
									if(kcollcount%2==0 && kcollcount > 0 )
									{
										if(optionState==0)
										{
											pointEasy+=100;
...
void iDraw()
{
	...
	//:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::GAME STATE::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
		if (gameState == -1)  
		{
			...
			iShowBMP2(565,10,"Pic\\muslogo.bmp",0);
			musicButton();
			iShowBMP2(120,320,"Pic\\pop2.bmp",255);                                            //Surname of Game
			for(int i=0;i<5;i++)
			{
				iShowBMP2(bCordinate[i].x,bCordinate[i].y,button[i],0);                        /// Show Button 
			}
		 ...
		 else if(gameState == 0)
		{
				       if(optionState == -1)
					   {
							...
							for(int i = 0; i < 2; i++)
							{
								iShowBMP2(opCordinate[i].opx,opCordinate[i].opy,optionbutton[i],0);                        /// Show Button 
							}
      ...
      else if(gameState == 1)
	{
						...
						if(hsoptionState == -1)
						{
							...
							for(int i = 0; i < 2; i++)
							{
								iShowBMP2(hsopCor[i].hsopx,hsopCor[i].hsopy,hsoptionbutton[i],0);                        /// Show Button 
							}
						 }

						else if(hsoptionState == 0)
						{
							iShowBMP(0,0,score);
							highScoreShow();
							iShowBMP2(exitx,exity,"Pic\\exit.bmp",0);
...
void iMouse(int button, int state, int mx, int my)
{
	...
	if (optionState ==- 1  && gameState==0)                                                                                                //***Debug ** (button will excute only in the home page)
	{
	 ...
	 if(gameState==1 && hsoptionState==-1)
	 {
		 for(int k=0;k<2;k++)
		 {
	...
	if(hsoptionState==0 || hsoptionState == 1)
	{
	...
	if(hsoptionState == -1 && gameState == 1 && hsopflag == 2)
	{
		if(mx >= exitx && mx <= exitx+100 && my >= exity && my <= exity+72)												
		{
			gameState = -1;
...
void iKeyboard(unsigned char key)
{
	...
	if (key == 'r' || key == 'R')
	{
		appendHighScore(pointEasy);
		appendHighScore1(pointHard);
		reset();
	}
...
int main()
{
	...
	iSetTimer(100,KhajanaChange);
	iSetTimer(400,KhajanaOp);
	SetBalloonEasy();
	SetBalloonHard();
	SetBombEasy();
	SetBombHard();
	...
	int sum=100;
	for(int i = 4; i >= 0; i--)									//Space for home button
	{
		bCordinate[i].x = 40;
		bCordinate[i].y = sum;
		sum += 50;
	...
	int opsum = 70;
	for(int i = 1; i >= 0; i--)									//Space for game option  button
	{
	...
	int hsopsum = 160;
	for(int i = 1; i >= 0; i--)									//Space for high Score option button
	{
		hsopCor[i].hsopx = 450;
